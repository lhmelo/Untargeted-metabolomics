...
sig.groupnames <- select(sig.groupnames, groupname)
...
#  
SigFeatures <- sig.groupnames %>% join(xset.filtered, by="groupname", type="left") %>% 
  select(-mzmin, -mzmax, -rt, -rtmin, -rtmax, -npeaks, -RT, -AveSmp) %>%
  join(OtherIons, by="MassFeature", type="left")
...
write.csv(NeutralMassList, paste(Fraction, "NeutralMassList.csv", sep="."), row.names = FALSE)
...
write.csv(UniqueNML, paste(Fraction, "UniqueNML.csv", sep="."), row.names = FALSE)
...
write.csv(UniqueIsoHypNM, paste(Fraction, "UniqueIsoHypNM.csv", sep="."), row.names = FALSE)
...
write.csv(UniqueHypNM, paste(Fraction, "UniqueHypNM.csv", sep="."), row.names = FALSE)
