...
qcplotter <- function(PPM=5){
  ...
  for (j in 1:length(FractionList)){
    ...
    Samples <-Samples[!grepl("Blank", Samples)]
    Blanks <- AllSamples[grepl("Blank", AllSamples)]
    ...
    Treat3 <- Samples[grepl(as.character(Treat3ID), Samples)]
    ...
    QCAppear[grepl(Treat3ID , AllSamples) , "Colors" ] <- "#99CC66FF" #Green with Alpha = 1
    QCAppear[grepl("Blank" , AllSamples, ignore.case = TRUE) , "Colors"] <- "#BEBEBEFF"
